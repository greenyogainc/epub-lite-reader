# Partner Center manual submission checklist — EPUB Lite Reader 1.0.5
...
- [ ] Run **WACK** on a clean machine/VM against `packages/EpubLiteReader-1.0.5-win-x64.msix`
...
- [ ] Optionally sideload-install the x64 package on a clean machine to smoke-test
      ...
      confirm an in-place **upgrade from 1.0.4 preserves** reading position, bookmarks, and settings.

## 1. Packages
- [ ] Create a new submission for product 9N2L3FKHCV5G.
- [ ] Upload `packages/EpubLiteReader-1.0.5-win-x64.msix`.
- [ ] Upload `packages/EpubLiteReader-1.0.5-win-arm64.msix`.
- [ ] Confirm Partner Center shows version **1.0.5.0** for both, architectures x64 and arm64.
...
- [ ] Paste the **What's new in 1.0.5** block into each language's "What's new in this version" field.
...
- [ ] Remove any screenshots from the previous submission so the set is not mixed.

## 5. Properties / declarations
- [ ] Confirm no new capabilities were added (manifest declares only `runFullTrust`; unchanged from 1.0.4).
- [ ] Data-collection / privacy answers unchanged from 1.0.4.
- [ ] Age rating questionnaire: unchanged from 1.0.4.
...
- Do not overwrite or relabel the existing 1.0.4 artifacts — 1.0.5 is a new submission of the same product.
